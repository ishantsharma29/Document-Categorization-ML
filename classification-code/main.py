...
def reorganize_dataset(path):
	...
	folders = glob.glob(path + '/*') #finds all the pathnames matching the pattern
	if len(folders) == 2: #finds the no. of files in the folder
		return
	else:
		...
		for like in likes:
			...
			for f in files:
				...
				newname = like + '_' + name  #renaming the file acc. to like and dislike
