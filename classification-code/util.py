...
def find_incompatible_files(path):
	"""
	Finds the filenames that are incompatible with  These files are usually not compatible with UTF8!
	parameter `path` is the absolute or relative path of the training data's root directory.
	returns a list of strings.
	"""

	count_vector = sklearn.feature_extraction.text.CountVectorizer() #converts a collection of text documents to a matrix if token counts and returns a sparse matrix
	files = sklearn.datasets.load_files(path)  #loas text files with categories as subfolder names
	num = []
