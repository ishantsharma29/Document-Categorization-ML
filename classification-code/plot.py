...
import main
import util
import matplotlib.pyplot as plt  #collection of command style fuction that make malplotic work like MATLAB. 
...
def KNN(path):
	...
	#main.reorganize_dataset(path)  #classify the train documents into likes and dislike 
	main.remove_incompatible_files(path)  #remove the files(UTF-8) that are incompatible with 'CountVectorizer'
	...
	#only finding TF here( by default use_idf is true, so assigning it to false helps to implement only TF)
	...
	TFIDF = vectorizer.fit_transform(files.data)
	print TFIDF.shape

	# KNN classifier is trained here
	n_neighbors = 5
	clf = sklearn.neighbors.KNeighborsClassifier(n_neighbors, weights='distance') 
	#weight points by the onverse of the distance. In this case, closer neighbours of a query point will have a greater influence than    neighbours which are further away
